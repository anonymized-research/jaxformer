...
class RemoteMaster:
        ...
        def connect_to_workers():
            ...
            with print_time(f'Connecting to {len(worker_addresses)} workers'):
                ...
                for i, (worker_ip, worker_port) in enumerate(worker_addresses):
                    ...
                    # TODO(anonymized): FN_INIT_CALL is not received without sleep, this should be needed
                    time.sleep(1)
    ...
    # TODO(anonymized): for pjit partition, we save each partition. For pjit xmap emulation, we only need to save global state once.
    @func_set_timeout(2000)
